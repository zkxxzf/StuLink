...
def clear_exam_data_cache():
    ...
    v1.18.1.0 审核修正：两个缓存都受 _exam_data_lock 保护，清空时同样持锁，
...
class ExamData:
    ...
    def totals_of(self, class_name=None, direction=None):
        if class_name:
            ...
            # v1.18.1.0 审核修正：不直接返回内部共享 list（_by_class 值），
...
def exam_score_means(grade, subjects, direction=None, class_name=None):
    ...
    with _exam_data_lock:
        ...
        if ent is not None and time.time() - ent[0] < _EXAM_DATA_TTL:
            # v1.18.1.0 审核修正：返回时拷贝外层与 means/counts 内层，
            # 避免 caller 就地修改污染进程级缓存（当前 caller 均为只读，修修正防未来变更）。
            cached = ent[1]
            return {eid: {**info,
                          'means': dict(info.get('means') or {}),
